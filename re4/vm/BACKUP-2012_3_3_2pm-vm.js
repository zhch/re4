...
bmp_lib.render = function(element, grid, opt_palette) {
	...
	{
		throw(new Error('bmp_lib.render: Invalid element: ' + element));
	} 
	...
	{
		throw(new Error('bmp_lib.render: Invalid HTML tag: ' + element.tagName));
	}
};
...
//	Thanks to Alexander Ivanov for efficiency improvements in this function.
	...
	if (number != 0) {
		throw(new Error('Overflow, number too big for string length'));
	}
	return string;
};
...
bmp_lib.encode64_.keyStr = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=';
